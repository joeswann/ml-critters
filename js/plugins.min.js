!function(window, undefined) {
...
}(window), function(main, modules, paths, Buffer, process, __require__, global) {
    function Module(filename, dirname) {
        this.id = filename, this.exports = {}, this.dirname = dirname, this.filename = filename, 
        this.loaded = !0;
    }
    function require(path) {
        var data, module = cache[path];
        return module || (data = modules[paths[path]], cache[path] = module = new Module(data[1], data[2]), 
        module.require = require, data[0].call(module.exports, __require__, require, module.exports, module.filename, module.dirname, module, process, Buffer, global)), 
        module.exports;
    }
    var isCommonJS = "undefined" != typeof module && module.exports, cache = {};
    isCommonJS || null === process || void 0 === process || 0 === process.argv.length && process.argv.push("browser", main), 
    require.resolve = function(path) {
        return path;
    }, Module.prototype.require = require, isCommonJS ? module.exports = require(main) : require(main);
}("./index.js", [ [ function(__require__, require, exports, __filename, __dirname, module) {
    var pathUtils = module.exports;
    pathUtils.removeEmpties = function(parts) {
        for (var i = parts.length; i--; ) parts[i] || parts.splice(i, 1);
        return parts;
    }, pathUtils.trim = function(parts) {
        for (var end, length = parts.length - 1, start = -1; start++ < length && "" === parts[start]; ) ;
        for (end = length + 1; end-- && "" === parts[end]; ) ;
        return start > end ? [] : parts.slice(start, end + 1);
    }, pathUtils.normalizeArray = function(parts, allowAboveRoot) {
        for (var last, i = parts.length, up = 0; i--; ) last = parts[i], "." === last ? parts.splice(i, 1) : ".." === last ? (parts.splice(i, 1), 
        up++) : up && (parts.splice(i, 1), up--);
        if (allowAboveRoot) for (;up--; ) parts.unshift("..");
        return parts;
    };
}, "path_utils", "../node_modules/url_path/node_modules/path_utils/src" ], [ function(__require__, require, exports, __filename, __dirname, module, process) {
    var pathUtils = require("path_utils"), urlPath = module.exports, IS_URL = /^(?:[a-z]+:)?\/\//i;
    urlPath.isAbsolute = function(str) {
        return "/" === str[0] || IS_URL.test(str);
    }, urlPath.isAbsoluteURL = function(str) {
        return IS_URL.test(str);
    }, urlPath.isURL = urlPath.isAbsoluteURL, urlPath.normalize = function(str) {
        var i, isAbs = urlPath.isAbsolute(str), trailingSlash = "/" === str[str.length - 1], segments = str.split("/"), nonEmptySegments = [];
        for (i = 0; i < segments.length; i++) segments[i] && nonEmptySegments.push(segments[i]);
        return str = pathUtils.normalizeArray(nonEmptySegments, !isAbs).join("/"), str || isAbs || (str = "."), 
        str && trailingSlash && (str += "/"), (isAbs ? "/" : "") + str;
    }, urlPath.resolve = function() {
        var i, str, resolvedPath = "", resolvedAbsolute = !1;
        for (i = arguments.length - 1; i >= -1 && !resolvedAbsolute; i--) {
            if (str = i >= 0 ? arguments[i] : process.cwd(), "string" != typeof str) throw new TypeError("Arguments to resolve must be strings");
            str && (resolvedPath = str + "/" + resolvedPath, resolvedAbsolute = "/" === str.charAt(0));
        }
        return resolvedPath = pathUtils.normalizeArray(pathUtils.removeEmpties(resolvedPath.split("/")), !resolvedAbsolute).join("/"), 
        (resolvedAbsolute ? "/" : "") + resolvedPath || ".";
    }, urlPath.relative = function(from, to) {
        from = resolve(from).substr(1), to = resolve(to).substr(1);
        var outputParts, i, il, fromParts = pathUtils.trim(from.split("/")), toParts = pathUtils.trim(to.split("/")), length = Math.min(fromParts.length, toParts.length), samePartsLength = length;
        for (i = 0; length > i; i++) if (fromParts[i] !== toParts[i]) {
            samePartsLength = i;
            break;
        }
        for (outputParts = [], i = samePartsLength, il = fromParts.length; il > i; i++) outputParts.push("..");
        return outputParts = outputParts.concat(toParts.slice(samePartsLength)), outputParts.join("/");
    }, urlPath.join = function() {
        var segment, i, il, str = "";
        for (i = 0, il = arguments.length; il > i; i++) {
            if (segment = arguments[i], "string" != typeof segment) throw new TypeError("Arguments to join must be strings");
            segment && (str += str ? "/" + segment : segment);
        }
        return urlPath.normalize(str);
    }, urlPath.dir = function(str) {
        return str = str.substring(0, str.lastIndexOf("/") + 1), str ? str.substr(0, str.length - 1) : ".";
    }, urlPath.dirname = urlPath.dir, urlPath.base = function(str, ext) {
        return str = str.substring(str.lastIndexOf("/") + 1), ext && str.substr(-ext.length) === ext && (str = str.substr(0, str.length - ext.length)), 
        str || "";
    }, urlPath.basename = urlPath.base, urlPath.ext = function(str) {
    ...
    function isString(obj) {
        var type = typeof obj;
        return "string" === type || obj && "object" === type && "[object String]" === toString.call(obj) || !1;
    }
    function isObject(obj) {
        var typeStr;
        return null === obj || void 0 === obj ? !1 : (typeStr = typeof obj, "function" === typeStr || "object" === typeStr);
    }
    function arrayMap(array, callback) {
        for (var i = -1, j = -1, length = array.length, result = []; ++i < length; ) result[++j] = callback(array[i], i, array);
        return result;
    }
    function Context() {
        this.require = null, this.exports = null, this.__filename = null, this.__dirname = null, 
        this.module = null, this.process = null, this.Buffer = null, this.global = null;
    }
    function Module(id, parent) {
        this.id = id, this.parent = parent, this.exports = {}, this.dirname = null, this.filename = null, 
        this.require = null, this.loaded = !1, this.children = [], this.__MODULE_PATH__ = MODULE_PATH, 
    ...
    function moduleRequire(path) {
        ...
        return load(path, this, !1, !1);
    }
    function createRequire(module) {
        function require(path) {
            return moduleRequire.call(module, require.resolve(path));
        }
        return require.resolve = function(path) {
            return resolveFilename(path, module);
        }, module.require = require, require;
    }
    function compile(module, content) {
        var context = new Context();
        context.require = createRequire(module), context.exports = module.exports, context.__filename = module.filename, 
        context.__dirname = module.dirname, context.module = module, context.process = process, 
        context.Buffer = Buffer, context.global = global;
        try {
            runInContext(content, context);
        } catch (e) {
            throw e.message = module.filename + ": " + e.message, e;
        }
    }
    function loadModule(module) {
        var content, filename = module.filename, ext = urlPath.ext(module.filename);
        if (".js" === ext) content = readFile(filename), compile(module, content); else {
            if (".json" !== ext) throw new Error("extension " + ext + " not supported");
            content = readFile(filename);
            try {
                module.exports = JSON.parse(content);
            } catch (e) {
                throw e.message = filename + ": " + e.message, e;
            }
        }
        module.loaded = !0;
    }
    function load(path, parent, isMain) {
        var filename = path, cache = Module._cache, module = cache[filename], failed = !0;
        if (!module) {
            module = new Module(filename, parent), module.filename = filename, module.dirname = urlPath.dir(filename), 
            isMain && (module.id = "."), cache[filename] = module;
            try {
                loadModule(module), failed = !1;
            } finally {
                failed && delete cache[filename];
            }
        }
    ...
    function exists(src) {
        var request;
        try {
            request = new global.XMLHttpRequest(), request.open("HEAD", src, !1), request.send(null);
        } catch (e) {
            return !1;
        }
        return 404 !== request.status;
    }
    function readFile(src) {
        var request, status;
        try {
            request = new global.XMLHttpRequest(), request.open("GET", src, !1), request.send(null), 
            status = request.status;
        } catch (e) {}
        return 200 === status || 304 === status ? request.responseText : null;
    }
    function resolveFilename(path, parent) {
        return MODULE_PATH = !1, urlPath.isAbsoluteURL(path) ? path : "." !== path[0] && "/" !== path[0] ? resolveNodeModule(path, parent) : (parent && (path = urlPath.join(parent.dirname, path)), 
        "/" === path[path.length - 1] && (path += "index.js"), hasExtension.test(path) || (path += ".js"), 
        path);
    }
    function resolveNodeModule(path, parent) {
        var pkg, found = !1, paths = path.match(MODULE_SPLITER), moduleName = paths[1], relativePath = paths[2], id = "node_modules/" + moduleName + "/package.json", depth = urlPath.join(process.cwd(), parent ? parent.dirname : "./").split(SPLITER).length, error = !1, root = parent ? parent.dirname : "./", resolved = parent.__MODULE_PATH__ ? urlPath.join(parent.__MODULE_PATH__, id) : id;
        for (exists(resolved) && (found = !0); !found && depth-- > 0; ) resolved = urlPath.join(root, id), 
        root += "/../", exists(resolved) && (found = !0);
        if (found) {
            try {
            ...
            if (MODULE_PATH = urlPath.dir(resolved), pkg && (resolved = urlPath.join(MODULE_PATH, parseMain(pkg))), 
            relativePath && (resolved = urlPath.join(urlPath.dir(resolved), relativePath), "/" === resolved[resolved.length - 1] && (resolved += "index.js"), 
            hasExtension.test(resolved) || (resolved += ".js"), !exists(resolved))) throw new Error("Cannot find module file " + resolved);
            hasExtension.test(resolved) || (resolved += ".js");
        } else error = !0;
        if (error) throw new Error("Module failed to find node module " + moduleName);
        return resolved;
    }
    function parseMain(pkg) {
        return isString(pkg.main) ? pkg.main : isString(pkg.browser) ? pkg.browser : "index";
    }
    ...
    var urlPath = require("url_path"), hasExtension = /\.(js|json)$/, MODULE_SPLITER = /([^/]*)(\/.*)?/, SPLITER = /[\/]+/, FUNC_REPLACER = /[\.\/\-\@]/g, MODULE_PATH = "", nativeKeys = Object.keys, toString = Object.prototype.toString, hasOwnProp = Object.prototype.hasOwnProperty, objectKeys;
    objectKeys = nativeKeys ? function(obj) {
        return isObject(obj) ? nativeKeys(obj) : [];
    } : function(obj) {
        var key, has = hasOwnProp, keys = [];
        if (!isObject(obj)) return keys;
        for (key in obj) has.call(obj, key) && keys.push(key);
        return keys;
    }, Module._cache = {}, Module.init = function(path, isGlobal) {
        var module;
        isGlobal ? (module = Module.repl(), module.require(path)) : load(resolveFilename(path), null, !0);
    }, Module.repl = function() {
        var filename = "./repl", cache = Module._cache, module = new Module("repl", void 0);
        return module.filename = filename, module.dirname = urlPath.dir(filename), global.require = createRequire(module), 
        global.module = module, cache[filename] = module, module.loaded = !0, module;
    }, module.exports = Module;
}, "./module.js", "." ], [ function(__require__, require, exports, __filename, __dirname, module, process, Buffer, global) {
    var length, arg, key, value, Module = require("./module.js"), scriptTag = "currentScript" in document ? document.currentScript : function() {
        var scripts = document.getElementsByTagName("script");
        return scripts[scripts.length - 1];
    }(), SPLITER = /[\s ]+/, getAttribute = function(name) {
        return scriptTag ? scriptTag.getAttribute(name) || scriptTag.getAttribute("data-" + name) || scriptTag.getAttribute("x-" + name) : "";
    }, hasAttribute = function(name) {
        return scriptTag ? !!(scriptTag.hasAttribute(name) || scriptTag.hasAttribute("data-" + name) || scriptTag.hasAttribute("x-" + name)) : !1;
    }, main = getAttribute("main"), args = getAttribute("argv"), env = getAttribute("env"), isGlobal = hasAttribute("global"), processEnv = process.env, i = -1;
    if (args && process.argv.push.apply(process.argv, args.split(SPLITER)), env && (env = env.split(SPLITER))) for (length = env.length; ++i < length; ) arg = (env[i] || "").split("="), 
    key = arg[0], value = arg[1], key && (null == value || processEnv[key] || (processEnv[key] = value));
    ...
    }), (isGlobal || !isGlobal && !main) && (global.process = process, global.Buffer = Buffer), 
    main ? Module.init(main, isGlobal) : Module.repl();
}, "./index.js", "." ] ], {
    path_utils: 0,
    url_path: 1,
    "./module.js": 2,
    "./index.js": 3
}, "undefined" != typeof Buffer ? Buffer : function() {
    function Buffer(subject, encoding) {
        if (!(this instanceof Buffer)) return new Buffer(subject, encoding);
        var i, il, type = typeof subject;
        if ("base64" === encoding && "string" === type) for (subject = trim(subject); subject.length % 4 !== 0; ) subject += "=";
        if ("number" === type) this.length = coerce(subject); else if ("string" === type) this.length = Buffer.byteLength(subject, encoding); else {
            if ("object" !== type || subject.length !== +subject.length) throw new Error("Buffer(subject, encoding): First argument needs to be a number, array or string.");
            this.length = coerce(subject.length);
        }
        if ("string" === type) this.write(subject, encoding); else if ("number" === type) for (i = 0, 
        il = this.length; il > i; i++) this[i] = 0;
        return this;
    }
    ...
    function checkInt(buffer, value, offset, ext, max, min) {
        ...
        if (value > max || min > value) throw new TypeError("value is out of bounds");
        if (offset + ext > buffer.length) throw new RangeError("index out of range");
    }
    function blitBuffer(out, src, offset, length) {
        for (var srcLength = src.length, outLength = out.length, i = 0; length > i && !(i + offset >= outLength || i >= srcLength); i++) out[i + offset] = src[i];
        return i;
    }
    function toHex(num) {
    ...
    function utf8ToBytes(str) {
        for (var start, b, h, j, length = str.length, byteArray = [], i = 0; length > i; i++) if (b = str.charCodeAt(i), 
        127 >= b) byteArray.push(b); else for (start = i, b >= 55296 && 57343 >= b && i++, 
        h = encodeURIComponent(str.slice(start, i + 1)).substr(1).split("%"), j = 0; j < h.length; j++) byteArray.push(parseInt(h[j], 16));
        return byteArray;
    }
    function asciiToBytes(str) {
        for (var byteArray = [], i = 0, il = str.length; il > i; i++) byteArray.push(255 & str.charCodeAt(i));
        return byteArray;
    }
    function utf16leToBytes(str) {
        for (var c, hi, lo, byteArray = [], i = 0, il = str.length; il > i; i++) c = str.charCodeAt(i), 
    ...
    function clamp(index, len, defaultValue) {
        return "number" != typeof index ? defaultValue : (index = ~~index, index >= len ? len : index >= 0 ? index : (index += len, 
        index >= 0 ? index : 0));
    }
    ...
    function trim(str) {
        return str.trim ? str.trim() : str.replace(trim_regex, "");
    }
    var base64, toString = Object.prototype.toString, isArray = Array.isArray || function(obj) {
        return "[object Array]" === toString.call(obj);
    };
    Buffer.Buffer = Buffer, Buffer.SlowBuffer = Buffer, Buffer.poolSize = 8192, Buffer.INSPECT_MAX_BYTES = 50, 
    Buffer.prototype.write = function(string, offset, length, encoding) {
        ...
        if (length ? (length = +length, length > remaining && (length = remaining)) : length = remaining, 
        encoding = (encoding || "utf8").toLowerCase(), "utf8" === encoding || "utf-8" === encoding) return this.utf8Write(string, offset, length);
        if ("ascii" === encoding || "raw" === encoding) return this.asciiWrite(string, offset, length);
        if ("binary" === encoding) return this.binaryWrite(string, offset, length);
        if ("ucs2" === encoding || "ucs-2" === encoding || "utf16le" === encoding || "utf-16le" === encoding) return this.utf16leWrite(string, offset, length);
        if ("hex" === encoding) return this.hexWrite(string, offset, length);
        if ("base64" === encoding) return this.base64Write(string, offset, length);
        throw new Error("Buffer.write(string, offset, length, encoding) Unknown encoding " + encoding);
    }, Buffer.prototype.copy = function(target, target_start, start, end) {
        if (start || (start = 0), end || 0 === end || (end = this.length), target_start || (target_start = 0), 
        end !== start && 0 !== target.length && 0 !== this.length) {
            if (start > end) throw new Error("Buffer.copy(target, target_start, start, end) sourceEnd < sourceStart");
            ...
            for (var i = 0, il = end - start; il > i; i++) target[i + target_start] = this[i + start];
        }
    }, Buffer.prototype.slice = function(start, end) {
        var sliceLen, len = this.length, buffer = new Buffer(sliceLen, void 0, !0), i = 0;
        for (start = clamp(start, len, 0), end = clamp(end, len, len), sliceLen = end - start; sliceLen > i; i++) buffer[i] = this[i + start];
        return buffer;
    }, Buffer.prototype.fill = function(value, start, end) {
        if (value || (value = 0), start || (start = 0), end || (end = this.length), start > end) throw new Error("Buffer.fill(value, start, end) end < start");
        ...
        var bytes, len, i = start;
        if ("number" == typeof value) for (i = start; end > i; i++) this[i] = value; else for (bytes = utf8ToBytes(value.toString()), 
        len = bytes.length, i = start; end > i; i++) this[i] = bytes[i % len];
        return this;
    }, Buffer.prototype.inspect = function() {
        for (var out = [], len = this.length, i = 0; len > i; i++) if (out[i] = toHex(this[i]), 
        i === Buffer.INSPECT_MAX_BYTES) {
            out[i + 1] = "...";
            break;
        }
        ...
        for (var jsonArray = [], i = this.length; i--; ) jsonArray[i] = this[i];
        return {
            type: "Buffer",
            data: jsonArray
        };
    }, Buffer.prototype.toArrayBuffer = function() {
        var buffer, i = this.length;
        if ("undefined" != typeof Uint8Array) for (buffer = new Uint8Array(i); i--; ) buffer[i] = this[i]; else for (buffer = []; i--; ) buffer[i] = this[i];
        return buffer;
    }, Buffer.prototype.toString = Buffer.prototype.toLocaleString = function(encoding, start, end) {
        if (encoding = (encoding || "utf8").toLowerCase(), start || (start = 0), end = null == end ? this.length : +end, 
        end === start) return "";
        if ("utf8" === encoding || "utf-8" === encoding) return this.utf8Slice(start, end);
        if ("ascii" === encoding || "raw" === encoding) return this.asciiSlice(start, end);
        if ("binary" === encoding) return this.binarySlice(start, end);
        if ("ucs2" === encoding || "ucs-2" === encoding || "utf16le" === encoding || "utf-16le" === encoding) return this.utf16leSlice(start, end);
        if ("hex" === encoding) return this.hexSlice(start, end);
        if ("base64" === encoding) return this.base64Slice(start, end);
        throw new Error("Buffer.write(string, offset, length, encoding) Unknown encoding " + encoding);
    }, Buffer.prototype.hexWrite = function(string, offset, length) {
        offset || (offset = 0);
        var strLen, b, i, remaining = this.length - offset;
        if (length ? (length = +length, length > remaining && (length = remaining)) : length = remaining, 
        strLen = string.length, strLen % 2 !== 0) throw new Error("Buffer.hexWrite(string, offset, length) Invalid hex string");
        for (length > strLen / 2 && (length = strLen / 2), i = 0; length > i; i++) {
            if (b = parseInt(string.substr(2 * i, 2), 16), isNaN(b)) throw new Error("Buffer.hexWrite(string, offset, length) Invalid hex string");
            this[offset + i] = b;
        }
        return i;
        ...
        for (var b, res = "", tmp = "", i = start; end > i; i++) b = this[i], 127 >= b ? (res += decodeUtf8Char(tmp) + String.fromCharCode(b), 
        tmp = "") : tmp += "%" + b.toString(16);
        return res + decodeUtf8Char(tmp);
    }, Buffer.prototype.base64Slice = function(start, end) {
        return base64.encode(0 === start && end === this.length ? this : this.slice(start, end));
    }, Buffer.prototype.asciiSlice = function(start, end) {
        end = Math.min(this.length, end);
        for (var ret = "", i = start; end > i; i++) ret += String.fromCharCode(this[i]);
        return ret;
    }, Buffer.prototype.binarySlice = Buffer.prototype.asciiSlice, Buffer.prototype.hexSlice = function(start, end) {
        var i, len = this.length, out = "";
        for ((!start || 0 > start) && (start = 0), (!end || 0 > end || end > len) && (end = len), 
        i = start; end > i; i++) out += toHex(this[i]);
        return out;
    }, Buffer.prototype.utf16leSlice = function(start, end) {
        for (var bytes = this.slice(start, end), i = 0, il = bytes.length, res = ""; il > i; i += 2) res += String.fromCharCode(bytes[i] + 256 * bytes[i + 1]);
        return res;
        ...
        return offset >>>= 0, noAssert || checkOffset(offset, 4, this.length), 16777216 * this[offset] + (this[offset + 1] << 16 | this[offset + 2] << 8 | this[offset + 3] >>> 0);
    }, Buffer.prototype.readInt8 = function(offset, noAssert) {
        offset >>>= 0, noAssert || checkOffset(offset, 1, this.length);
        var val = this[offset];
        return 128 & val ? -1 * (255 - val + 1) : val;
        ...
        this[offset] = value, this[offset + 1] = value >>> 8, offset + 2;
    }, Buffer.prototype.writeUInt16BE = function(value, offset, noAssert) {
        return value = +value, offset >>>= 0, noAssert || checkInt(this, value, offset, 2, 65535, 0), 
        this[offset] = value >>> 8, this[offset + 1] = value, offset + 2;
        ...
        this[offset] = value, this[offset + 1] = value >>> 8, this[offset + 2] = value >>> 16, 
        ...
        return "string" != typeof encoding ? !1 : (encoding = encoding.toLowerCase(), "utf8" === encoding || "utf-8" === encoding || "hex" === encoding || "base64" === encoding || "ascii" === encoding || "binary" === encoding || "ucs2" === encoding || "ucs-2" === encoding || "utf16le" === encoding || "utf-16le" === encoding || "raw" === encoding);
    }, Buffer.byteLength = function(str, encoding) {
        if (str += "", encoding = (encoding || "utf8").toLowerCase(), "utf8" === encoding || "utf-8" === encoding) return utf8ToBytes(str).length;
        if ("ascii" === encoding || "binary" === encoding || "raw" === encoding) return str.length;
        if ("ucs2" === encoding || "ucs-2" === encoding || "utf16le" === encoding || "utf-16le" === encoding) return 2 * str.length;
        if ("hex" === encoding) return str.length >>> 1;
        if ("base64" === encoding) return base64ToBytes(str).length;
        ...
        var buffer, postion, item, i, il;
        if (void 0 === totalLength) for (totalLength = 0, i = 0, il = list.length; il > i; i++) totalLength += list[i].length;
        for (buffer = new Buffer(totalLength), postion = 0, i = 0, il = list.length; il > i; i++) item = list[i], 
        item.copy(buffer, postion), postion += item.length;
        return buffer;
    }, Buffer.compare = function(a, b) {
        if (!Buffer.isBuffer(a) || !Buffer.isBuffer(b)) throw new Error("Buffer.compare(a, b) Arguments must be Buffers");
        for (var x = a.length, y = b.length, i = 0, il = Math.min(x, y); il > i && a[i] === b[i]; ) i++;
        return i !== il && (x = a[i], y = b[i]), y > x ? -1 : x > y ? 1 : 0;
    }, base64 = function() {
        function decode(str) {
            var code = str.charCodeAt(0);
            return code === PLUS ? 62 : code === SLASH ? 63 : NUMBER > code ? -1 : NUMBER + 10 > code ? code - NUMBER + 26 + 26 : UPPER + 26 > code ? code - UPPER : LOWER + 26 > code ? code - LOWER + 26 : -1;
        }
        function encode(num) {
        ...
        var ArrayType = "undefined" != typeof Uint8Array ? Uint8Array : Array, LOOK_UP = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/", PLUS = "+".charCodeAt(0), SLASH = "/".charCodeAt(0), NUMBER = "0".charCodeAt(0), LOWER = "a".charCodeAt(0), UPPER = "A".charCodeAt(0);
        return {
            decode: function(str) {
                if (str.length % 4 > 0) throw new Error("base64.decode(str) Invalid string. Length must be a multiple of 4");
                var i, j, l, L, tmp, placeHolders, array, len = str.length;
                for (placeHolders = "=" === str.charAt(len - 2) ? 2 : "=" === str.charAt(len - 1) ? 1 : 0, 
                array = new ArrayType(3 * str.length / 4 - placeHolders), l = placeHolders > 0 ? str.length - 4 : str.length, 
                L = 0, i = 0, j = 0; l > i; i += 4, j += 3) tmp = decode(str.charAt(i)) << 18 | decode(str.charAt(i + 1)) << 12 | decode(str.charAt(i + 2)) << 6 | decode(str.charAt(i + 3)), 
                array[L++] = (16711680 & tmp) >> 16, array[L++] = (65280 & tmp) >> 8, array[L++] = 255 & tmp;
                return 2 === placeHolders ? (tmp = decode(str.charAt(i)) << 2 | decode(str.charAt(i + 1)) >> 4, 
                array[L++] = 255 & tmp) : 1 === placeHolders && (tmp = decode(str.charAt(i)) << 10 | decode(str.charAt(i + 1)) << 4 | decode(str.charAt(i + 2)) >> 2, 
                array[L++] = tmp >> 8 & 255, array[L++] = 255 & tmp), array;
            },
            encode: function(uint8) {
                var temp, length, i, extraBytes = uint8.length % 3, output = "";
                for (i = 0, length = uint8.length - extraBytes; length > i; i += 3) temp = (uint8[i] << 16) + (uint8[i + 1] << 8) + uint8[i + 2], 
                output += tripletToBase64(temp);
                return 1 === extraBytes ? (temp = uint8[uint8.length - 1], output += encode(temp >> 2), 
                output += encode(temp << 4 & 63), output += "==") : 2 === extraBytes && (temp = (uint8[uint8.length - 2] << 8) + uint8[uint8.length - 1], 
                output += encode(temp >> 10), output += encode(temp >> 4 & 63), output += encode(temp << 2 & 63), 
                output += "="), output;
            }
        };
    }();
    ...
    function EventObject(listener, ctx) {
        this.listener = listener, this.ctx = ctx;
    }
    function EventEmitter() {
        this._events = Object.create(null), this._maxListeners = EventEmitter.defaultMaxListeners;
    }
    function emit(eventList, args) {
        var a1, a2, a3, a4, event, i, length = args.length;
        if (1 === length) for (i = eventList.length; i--; ) (event = eventList[i]) && event.listener.call(event.ctx); else if (2 === length) for (a1 = args[1], 
        i = eventList.length; i--; ) (event = eventList[i]) && event.listener.call(event.ctx, a1); else if (3 === length) for (a1 = args[1], 
        a2 = args[2], i = eventList.length; i--; ) (event = eventList[i]) && event.listener.call(event.ctx, a1, a2); else if (4 === length) for (a1 = args[1], 
        a2 = args[2], a3 = args[3], i = eventList.length; i--; ) (event = eventList[i]) && event.listener.call(event.ctx, a1, a2, a3); else if (5 === length) for (a1 = args[1], 
        a2 = args[2], a3 = args[3], a4 = args[4], i = eventList.length; i--; ) (event = eventList[i]) && event.listener.call(event.ctx, a1, a2, a3, a4); else for (arrayShift.apply(args), 
        i = eventList.length; i--; ) (event = eventList[i]) && event.listener.apply(event.ctx, args);
    }
    function Process() {
        EventEmitter.call(this), this.pid = 0, this.title = "browser", this.env = {}, this.argv = [], 
        this.version = "1.0.0", this.versions = {}, this.config = {}, this.execPath = ".", 
        this.execArgv = [], this.arch = global.navigator ? ((/\b(?:AMD|IA|Win|WOW|x86_|x)[32|64]+\b/i.exec(global.navigator.userAgent) || "")[0] || "unknown").replace(/86_/i, "").toLowerCase() : "unknown", 
        this.platform = global.navigator ? (global.navigator.platform ? global.navigator.platform.split(/[ \s]+/)[0] : "unknown").toLowerCase() : "unknown", 
        this.maxTickDepth = 1e3, this._cwd = global.location ? global.location.pathname : "/";
    }
    var global = new Function("return this;")(), arrayShift = Array.prototype.shift, hasOwnProperty = Object.prototype.hasOwnProperty;
    return EventEmitter.prototype.on = function(type, listener, ctx) {
        if ("function" != typeof listener) throw new TypeError("EventEmitter.on(type, listener[, ctx]) listener must be a function");
        var events = this._events, eventList = events[type] || (events[type] = []), maxListeners = this._maxListeners;
        return eventList.push(new EventObject(listener, ctx || this)), -1 !== maxListeners && eventList.length > maxListeners && console.error("EventEmitter.on(type, listener, ctx) possible EventEmitter memory leak detected. " + maxListeners + " listeners added"), 
        this;
    }, EventEmitter.prototype.addListener = EventEmitter.prototype.on, EventEmitter.prototype.once = function(type, listener, ctx) {
        function once() {
            _this.off(type, once, ctx);
            var length = arguments.length;
            return 0 === length ? listener.call(ctx) : 1 === length ? listener.call(ctx, arguments[0]) : 2 === length ? listener.call(ctx, arguments[0], arguments[1]) : 3 === length ? listener.call(ctx, arguments[0], arguments[1], arguments[2]) : 4 === length ? listener.call(ctx, arguments[0], arguments[1], arguments[2], arguments[3]) : 5 === length ? listener.call(ctx, arguments[0], arguments[1], arguments[2], arguments[3], arguments[4]) : listener.apply(ctx, arguments);
        }
        var _this = this;
        return ctx || (ctx = this), this.on(type, once, ctx);
    }, EventEmitter.prototype.listenTo = function(obj, type, listener, ctx) {
        if (!hasOwnProperty.call(obj, "on") || "function" != typeof obj.on) throw new TypeError("EventEmitter.listenTo(obj, type, listener, ctx) obj must have a on function taking (type, listener[, ctx])");
        return obj.on(type, listener, ctx || this), this;
    }, EventEmitter.prototype.off = function(type, listener, ctx) {
        var eventList, event, i, events = this._events;
        if (!type) return this.removeAllListeners();
        if (eventList = events[type], !eventList) return this;
        if (listener) {
            for (ctx = ctx || this, i = eventList.length; i--; ) event = eventList[i], event.listener === listener && (this.emit("removeListener", type, event.listener, event.ctx), 
            eventList.splice(i, 1));
            0 === eventList.length && delete events[type];
        } else {
            for (i = eventList.length; i--; ) event = eventList[i], this.emit("removeListener", type, event.listener, event.ctx);
            eventList.length = 0, delete events[type];
        }
        return this;
    }, EventEmitter.prototype.removeListener = EventEmitter.prototype.off, EventEmitter.prototype.removeAllListeners = function() {
        var eventList, event, i, type, events = this._events;
        for (type in events) if (eventList = events[type]) {
            for (i = eventList.length; i--; ) event = eventList[i], this.emit("removeListener", type, event.listener, event.ctx);
            eventList.length = 0, delete events[type];
        }
        return this;
        ...
        return eventList && eventList.length ? (emit(eventList, arguments), this) : this;
        ...
        return this._maxListeners = 0 > value ? -1 : value, this;
    }, EventEmitter.defaultMaxListeners = 10, EventEmitter.listeners = function(obj, type) {
        if (null == obj) throw new TypeError("EventEmitter.listeners(obj, type) obj required");
        var eventList = obj._events && obj._events[type];
        return eventList ? eventList.slice() : [];
    }, EventEmitter.listenerCount = function(obj, type) {
        if (null == obj) throw new TypeError("EventEmitter.listenerCount(obj, type) obj required");
        var eventList = obj._events && obj._events[type];
        return eventList ? eventList.length : 0;
    }, EventEmitter.setMaxListeners = function(value) {
        if ((value = +value) !== value) throw new TypeError("EventEmitter.setMaxListeners(value) value must be a number");
        return EventEmitter.defaultMaxListeners = 0 > value ? -1 : value, value;
    }, EventEmitter.extend = function(child, parent) {
        return parent || (parent = this), child.prototype = Object.create(parent.prototype), 
        child.prototype.constructor = child, child._super = parent.prototype, child.extend = parent.extend, 
        child;
    }, EventEmitter.extend(Process), Object.defineProperty(Process.prototype, "browser", {
        get: function() {
            return !0;
        }
    }), Process.prototype.memoryUsage = function() {
        var performance = global.performance || {}, memory = {
            rss: 0,
            heapTotal: 0,
            heapUsed: 0
        };
        return performance.memory || (performance.memory = {}), function() {
            return memory.rss = performance.memory.jsHeapSizeLimit || 0, memory.heapTotal = performance.memory.totalJSHeapSize || 0, 
            memory.heapUsed = performance.memory.usedJSHeapSize || 0, memory;
        };
    }(), Process.prototype.nextTick = function() {
        var queue, canSetImmediate = !!global.setImmediate, canMutationObserver = !!global.MutationObserver, canPost = global.postMessage && global.addEventListener;
        if (canSetImmediate) return function(fn) {
            return global.setImmediate(fn);
        };
        if (queue = [], canMutationObserver) {
            var hiddenDiv = document.createElement("div"), observer = new MutationObserver(function() {
                var queueList = queue.slice(), length = queueList.length, i = -1;
                for (queue.length = 0; ++i < length; ) queueList[i]();
            });
        ...
        return canPost ? (global.addEventListener("message", function(e) {
            var fn, source = e.source;
            source !== global && null !== source || "process-tick" !== e.data || (e.stopPropagation(), 
            queue.length > 0 && (fn = queue.shift())());
        }, !0), function(fn) {
            queue.push(fn), global.postMessage("process-tick", "*");
        ...
        var length, newDir, cwd = global.location ? global.location.pathname : "/";
        if ("/" === dir ? newDir = "/" : (length = cwd.length, newDir = dir.length >= length ? dir : dir.substring(0, length) + "/"), 
        0 !== cwd.indexOf(newDir)) throw new Error("process.chdir can't change to directory " + dir);
        this._cwd = dir;
    }, Process.prototype.hrtime = function() {
        function performanceNow() {
            return start + performance.now();
        }
        var start, performance = global.performance || {};
        return Date.now || (Date.now = function() {
            return new Date().getTime();
        }), start = Date.now(), performance.now || (performance.now = performance.mozNow || performance.msNow || performance.oNow || performance.webkitNow || function() {
            return Date.now() - start;
        }), function(previousTimestamp) {
            var clocktime = .001 * performanceNow(), seconds = Math.floor(clocktime), nanoseconds = clocktime % 1 * 1e9;
            return previousTimestamp && (seconds -= previousTimestamp[0], nanoseconds -= previousTimestamp[1], 
            0 > nanoseconds && (seconds--, nanoseconds += 1e9)), [ seconds, nanoseconds ];
        };
    }(), Process.prototype.uptime = function() {
        var start = Date.now();
        return function() {
            return .001 * (Date.now() - start) | 0;
        };
